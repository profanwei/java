...
//		此处使用		
		...
		String machsql="select machineid,machinename from machinfo where dept_id='"+deptId1+"'";
		try {
			 list1=Sqlhelper.exeQueryList1(machsql);
		} catch (Exception e2) {
			// TODO 自动生成的 catch 块
			e2.printStackTrace();
		}
		for(int i=0;i<list1.get(0).size();i++) {
			machineNames.add(list1.get(0).get(i));
			machineIds.add(list1.get(1).get(i));
		}
		/*
		 * if(deptId1.equals("550")){ machineNames.add("数控电火花成形机床");
...
//	      
	        if(isRepairNum>0)  {
	        	machineBean.setState(2);
	        	 machineBean.setDescription("异常");
	        }else {
	        	machineBean.setState(1);
	        	 machineBean.setDescription("");
	        }
