...
//		参数1 表示重置密码，xiem
		...
		if(!para.isEmpty()&&para.equals("1")){
			...
			authoritySql = "update sysusers set PASSWORD='"+newPassWord+"',changePerson='"+changePerson+"'," +
	    ...
	    if (StringUtil.isNullOrEmpty(password)||StringUtil.isNullOrEmpty(newpassword)||StringUtil.isNullOrEmpty(Confirmpassword)) {
			...
			if (!StringUtil.isNullOrEmpty(password)&&sessionPassword.equals(password)) {		//填写了密码,且正确
				authoritySql = "update sysusers set PASSWORD='"+newpassword+"', OLD_PASSWORD='"+sessionPassword+"',changePerson='"+changePerson+"'," +
					"changeTime=to_date('"+changeTime+"','yyyy-mm-dd,hh24:mi:ss'),PWD_MODIFY_TIME=to_date('"+changeTime+"','yyyy-mm-dd,hh24:mi:ss') " +
					"where USER_ID='"+userId+"'";
			}else {
				String json = "{\"result\":\"密码错误!\"}";
				response.setCharacterEncoding("UTF-8");
				response.getWriter().append(json).flush();
				return;
