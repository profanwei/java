...
//		使用分页及其自动排序进行查询
		...
		List<BookOrderMachine> list = bookOrdeMapper.selectBookOrderMachineListByOrderId2(orderId);
		String json = DaiUtils.returnMiniUiJson(list);
		System.out.println("这里是"+json);
		return json;
	}
	public String returnMybookMachine3(String orderId, int pageNO, int pageSize, String sortField, String sortOrder) {
	/*	int one =countPerPage*pageNO;
		int two =(pageNO-1)*countPerPage+1;*/
//		使用分页及其自动排序进行查询
		PageHelper.startPage(pageNO,pageSize,DaiUtils.jspNameToOralceName(sortField.equals("")?"unid":sortField)+" "+DaiUtils.jspNameToOralceName(sortOrder));
		List<BookOrderMachine> list = bookOrdeMapper.selectBookOrderMachineListByOrderId3(orderId);
		String json = DaiUtils.returnMiniUiJson(list);
		System.out.println("这里是"+json);
		return json;
	}
	public String returnMybookMachine2(String orderId, int pageNO, int pageSize, String sortField, String sortOrder) {
	/*	int one =countPerPage*pageNO;
		int two =(pageNO-1)*countPerPage+1;*/
//		使用分页及其自动排序进行查询
		PageHelper.startPage(pageNO,pageSize,DaiUtils.jspNameToOralceName(sortField.equals("")?"unid":sortField)+" "+DaiUtils.jspNameToOralceName(sortOrder));
...
//			String json = "{\"total\":"+count+",\"data\":"+jsonData+"}";
	...
	public String returnMaxOrderId(String begin,String end) {
		System.out.println(begin+"kaishi");
		Machine ma=bookOrdeMapper.selectMaxOrder(begin,end);
	 ma.setDeptId(bookOrdeMapper.selectMaxDept(begin,end).getDeptId());

	
		 String json = PluSoft.Utils.JSON.Encode(ma);
		 System.out.println(json);
		return json;
	

}
	public String returnGante(String date) {
     String json="11";
     System.out.println(date);
     String dt="07:59:00";
     Date now=new Date();
     SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
     MachineGante mg=new MachineGante();
     MachineGante temp=new MachineGante();
     MachineGante mg1=new MachineGante();   
     temp.setStatus("start");
     System.out.println(temp.getStatus());
     mg.setCount(0);
     mg1.setCount(0);
     List<MachineGante> list =new ArrayList<>();
     Date end =new Date();
     ...
     long t=(end.getTime()-begin.getTime())/60000;
     ...
     for(int i=1;i<=t;i++){
    	    try {
    	     ...
    	     System.out.println(dt);
    	      mg=bookOrdeMapper.returnGante(date,dt);

   	     if(mg!=null&&mg.getStatus().equals("open")) {
               if(temp!=null&&(temp.getStatus().equals("open")||temp.getStatus().equals("start"))) {
         	      System.out.println(mg.getStatus());
            	   System.out.println(mg.getStatus());
            	   mg1.setCount(mg1.getCount()+1);
            	   mg1.setStatus("open");
               }else {
            	   
            	   MachineGante mg2=new MachineGante();
            	   mg2.setCount(mg1.getCount());
            	   mg2.setStatus(mg1.getStatus());
            	   list.add(mg2);
            	   mg1.setCount(1);
            	   mg1.setStatus("open");
               }
   	     ...
   	     else if(mg==null||mg.getStatus().equals("close")) {
   	    	 if(temp==null||temp.getStatus().equals("close")||temp.getStatus().equals("start")) {

          	   mg1.setCount(mg1.getCount()+1);
          	   System.out.println(mg1.getCount());
          	   mg1.setStatus("close");
             }else {
            	 MachineGante mg2=new MachineGante();
            	  mg2.setCount(mg1.getCount());
     ...
     json=DaiUtils.returnMiniUiJson(list);
     System.out.println(json);
	return json;	
	}
	public String returnGanteFanuc(String date) {
		String json="11";
		System.out.println(date);
		String dt="07:59:00";
		Date now=new Date();
		...
		for(int i=1;i<=t;i++){
			...
			dt=sdf.format(now);
			System.out.println(dt);
			mg=bookOrdeMapper.returnGanteFanuc(date,dt);
			
			if(mg!=null&&mg.getStatus().equals("开机")) {
				if(temp!=null&&(temp.getStatus().equals("开机")||temp.getStatus().equals("start"))) {
					System.out.println(mg.getStatus());
					System.out.println(mg.getStatus());
					mg1.setCount(mg1.getCount()+1);
					mg1.setStatus("开机");
			...
			else if(mg==null||mg.getStatus().equals("关机")) {
				if(temp==null||temp.getStatus().equals("关机")||temp.getStatus().equals("start")) {
	...
	public String returnMyBookOrder11(String orderId,String staffCode,int pageNo, int pageSize,String sortField,String sortOrder, String bookStatus, String companyId, String companyName) {
//		int one =countPerPage*pageNo;
//		int two =(pageNo-1)*countPerPage+1;
		 sortField=sortField.equals("")?"orderId":sortField;
		 ...
		 sortOrder=sortOrder.equals("")?"desc":sortOrder;
		/*
		 * String orderBy=DaiUtils.jspNameToOralceName(sortField)+" "+DaiUtils.
		 * jspNameToOralceName(sortOrder);
		 */
		 String orderBy="a."+sortField+" "+DaiUtils.jspNameToOralceName(sortOrder);
		 PageHelper.startPage(pageNo,pageSize,orderBy);
		 List<Order> list = bookOrdeMapper.selectBookOrderList11(orderId,staffCode,bookStatus,companyId,companyName);
//		 int count = bookOrdeMapper.selectBookOrderCount(staffCode);
...
//		然后进行对比，直至相等，就将当前的状态置为下一个！但是有个12.5的状态，代表审核中！
	...
	public String deleteAuditingBookOrder(String unid, String orderId) throws Exception {
		...
		int count = bookOrdeMapper.deleteAuditingBookOrderMapper(unid,orderId);
...
//	
		...
		public String deleteAuditingOrder(String orderId,String bookStatus) {
			int count1;
			int count2;
			if(bookStatus=="0"||bookStatus.equals("0")) {
				System.out.println(bookStatus);
			 count1 = bookOrdeMapper.deleteOrder(orderId);
			}else {
				 count1 = bookOrdeMapper.deleteAuditingOrderMapper(orderId);
				 count2 = bookOrdeMapper.deleteAuditingOrderbookingInfoMapper(orderId);
				
			}
			String jsonData="";
			System.out.println("count是"+count1);
			if (count1>=1) {
				jsonData="操作成功";
		 ...
		 * @return
		 */
		public String returnTime(String orderId) {
			System.out.println("nihao"+orderId);
			String time =bookOrdeMapper.selectTime(orderId);
			Date a = null;
			Date b = null;
			String json;
		 ...
		 * @return
		 */
		public String returnNum(String orderId) {
			String json;
			int count=bookOrdeMapper.selectNum(orderId);
			if(count==0) {
				json="可以";
			}else {
				json="不可以";
...
//		      
		        if(isRepairNum>0)  {
		        	...
		        	machBookDateline.setDescription("异常");
		        }else {
		        	machBookDateline.setState(1);
		        	machBookDateline.setDescription("");
		        }
		        ...
		        System.out.println("范伟"+sessionsList);
...
//………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………………	        
		...
		public String returnDept() {
         ...
         list=bookOrdeMapper.returndept();
         String jsonData = PluSoft.Utils.JSON.Encode(list);
         System.out.println(jsonData);
         String json = "{\"content\":"+jsonData+",\"no\":2000}";
         System.out.println(json);
         return json;
			
		}
		public String getDept(String machineId) {
			
			Dept dept;
			dept=bookOrdeMapper.getdept(machineId);
			String jsonData = PluSoft.Utils.JSON.Encode(dept);
			String json = "{\"content\":"+jsonData+"}";
			System.out.println(json);
			return json;
		...
		public String returnBookMachByMachService(String machineId, String deptId) {
		  ...
		  List<Machine> machinfo= bookOrdeMapper.returnmachinfo(deptId);

		  for(int j=0;j<machinfo.size();j++) {
			  machineIds.add(machinfo.get(j).getMachineId());
		  }
		  for(int k=0;k<machinfo.size();k++) {
		  machineNames.add(machinfo.get(k).getMachineName()); }
		 
			
		
		/*
		 * if(deptId.equals("0560")){ machineNames.add("数控电火花成形机床");
		 * machineNames.add("数控低速走丝电火花线切割"); machineNames.add("数控高速成型磨床");
		 * machineNames.add("超精密成形平面磨床"); machineNames.add("数控车床");
		 * machineNames.add("CNC雕刻机"); machineIds.add("2001001761");
		 * machineIds.add("2005011533"); machineIds.add("2013000822");
		 * machineIds.add("2016005139"); machineIds.add("SK50P");
		 * machineIds.add("2008001393");
		 * 
		 * }else{
		 * 
		 * 
		 * machineNames.add("车铣复合加工中心"); machineNames.add("超高速磨床");
		 * machineNames.add("超声辅助高速加工中心"); machineNames.add("高速五坐标加工中心");
		 * machineNames.add("加工中心"); machineIds.add("2018002921");
		 * machineIds.add("2007004971"); machineIds.add("2012000271");
		 * machineIds.add("2001001763"); machineIds.add("VL2");
		 * 
		 * }
		 */
...
//			使用分页及其自动排序进行查询
		...
		public String returnCompletedBookMachine11(String orderId, int pageNo, int pageSize, String sortField,
				String sortOrder) {
//			使用分页及其自动排序进行查询
			
			PageHelper.startPage(pageNo,pageSize,DaiUtils.jspNameToOralceName(sortField.equals("")?"unid":sortField)+" "+DaiUtils.jspNameToOralceName(sortOrder));
			List<BookOrderMachine> list = bookOrdeMapper.completedBookMapper11(orderId);
			String json = DaiUtils.returnMiniUiJson(list);
			return json;
		}
...
//		 * 此service针对的是可以不断保存数据，同时使用事务增强健壮性！
		...
		/* bookOrdeMapper.deleteCompletedBookingInfoMapper(orderId); */
			for (HashMap<String,String> hashMap : saveList) {
				...
				bookOrdeMapper.updatebookinginfo(hashMap);
//				String string =  hashMap.get("bookState");
//				/*System.out.println(string);*/
			}
		}
			public void saveCompletedBookingInfoService1(ArrayList<HashMap<String, String>> saveList,String orderId) throws Exception{
				...
				for (HashMap<String,String> hashMap : saveList) {
					...
					hashMap.put("orderId", orderId);
					bookOrdeMapper.saveCompletedBookingInfoMapper1(hashMap);
					bookOrdeMapper.saveCompletedBookingInfo(hashMap);
...
//					/*System.out.println(string);*/
		...
		/*	for (String saveString : saveList) {
			...
			public String saveCompletedBookingInfoService5(ArrayList<HashMap<String, String>> saveList,String orderId) throws Exception{
				...
				for (HashMap<String,String> hashMap : saveList) {
					String unid=hashMap.get("unid");
					String timeYmd=hashMap.get("timeYmd");
					
					if(unid.equals("")||timeYmd.equals("")) {
						 jsonData="禁止为空";
							return jsonData;
					}
				int count=bookOrdeMapper.selectBookOrderInfoIsPassCount3(hashMap);
				if(count==1) {
					 jsonData="已上报";
					return jsonData;
...
//					/*System.out.println(string);*/
	  ...
	  public void saveCompletedBookingInfoService2(ArrayList<HashMap<String,String>> saveList, String orderId) throws Exception{
	...
	for (HashMap<String,String> hashMap : saveList) {
		  ...
		  hashMap.put("orderId", orderId);
		  bookOrdeMapper.saveCompletedBookingInfo2(hashMap);
	
	  // String string = hashMap.get("bookState"); // /*System.out.println(string);
		...
		public void completedOrderAuditService(String orderId, String completedAdvice, String bookStatus, String staffCode) {
			// TODO Auto-generated method stub
			bookOrdeMapper.updatebookinginfo1(orderId);
			bookOrdeMapper.updatebookinginfo2(orderId);
			bookOrdeMapper.updatebookinginfo5(orderId);
			bookOrdeMapper.updatebookinginfo6(orderId);
			int isPassCount = bookOrdeMapper.selectBookOrderInfoIsPassCount1(orderId);
		/*
		 * int passStateCount=bookOrdeMapper.selectBookOrderInfoIsPassCount2(orderId);
		 */
			if(isPassCount==0) {
			bookOrdeMapper.completedOrderAuditMapper(orderId,completedAdvice,bookStatus,staffCode);
			}else {
				
				bookOrdeMapper.completedOrderAuditMapper1(orderId,completedAdvice,bookStatus,staffCode);
			}
		}


		public void completedOrderAuditService1(String orderId, String completedAdvice, String bookStatus, String staffCode) {
			// TODO Auto-generated method stub
			bookOrdeMapper.updatebookinginfo3(orderId);
			bookOrdeMapper.updatebookinginfo4(orderId);
			bookOrdeMapper.updatebookinginfo7(orderId);
			bookOrdeMapper.updatebookinginfo8(orderId);
...
//					然后同时更新ordersMachinesAudits的中的状态，更改为不通过，不需要其他的设置了，因为这个订单不可能在进行预定这个机床了，这个基本的上不会发生 ，就算是发生也会给他重新换个时间的！！！
		 ...
		 */
		@Transactional
		public void saveAuditAdviceService(ArrayList<HashMap<String, String>> saveList, String orderId,String createtime) throws Exception{
...
//			然后分析list数据，遍历一遍，然后先进行全部更新，然后进行判断 否，多加一条信息，插入另一个表，删除原有信息！---目前没有按照这个写
			for (HashMap<String,String> hashMap : saveList) {
				...
				hashMap.put("orderId", orderId);
				String pass=hashMap.get("isPass");
				String unid=hashMap.get("unid");
				
				
				System.out.println(pass+unid);				
...
//			此处需要一个判断，是否全部审核完毕，没有空的值！！！ 
			...
			if(isPassCount==0 && panduan.equals("通过")) bookOrdeMapper.updateOrderStatus(orderId,"13");
			if(isPassCount==0 && panduan.equals("不通过")) bookOrdeMapper.updateOrderStatus(orderId,"14");
