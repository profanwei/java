...
RGanttSchedule = function (gantt) {
    ...
    gantt.on("barcreate", this.__OnBarCreate, this);
}
RGanttSchedule.prototype = {

    __OnBarCreate: function (e) {
        var task = e.item,
            node = e.node,
            gantt = this.gantt;
        
        if (gantt.OrderType == "day") {
            task.Start = clearTime(task.Start);
            task.Finish = maxTime(task.Finish);
            var days = parseInt((task.Finish - task.Start) / (3600 * 24 * 1000));
            task.Duration = (days + 1) * (3600 * 24);
        } else if (gantt.OrderType == "hour") {
            task.Start = clearHours(task.Start);
            task.Finish = maxHours(task.Finish);
