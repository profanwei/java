	       ...
	       <tr>
	         <td>订单状态：</td>
	          <td><input id="bookStatus" name="bookStatus" class="mini-combobox" width="100" textName="" textField="text" valueField="id"
  				url="data/bookStatusComplete.txt"  allowInput="false" showNullItem="true" nullItemText="全部"  onvaluechanged="loadgrid"/>
	          </td> 
	          ...
	          <td colspan="2"><input id="companyName" name="companyId" class="mini-buttonedit" width="100%" showClose="true" oncloseclick="onCloseClick('companyName')"
            		onbuttonclick="onButtonEdit2" textName="companyName" required="false" value="" text=""   allowInput="false"/>
	          </td>
	          <td><input value="查找" type="button" onclick="loadgrid()" style="width:50px;"/></td>
	   		</tr>
	   		<input id="key" name="key"  class="mini-textbox" required="true" value="2" visible="false"/>
	   			<input  name="sign"  class="mini-textbox" required="true" value="uploadSelect" visible="false"/>
...
//          str += "</span>";
		...
		function ondMenu(orderId,companyName,connectorTel,deptUser,bookStatus,createTime,connector){
	    	...
	    	window.location="ResearchTask/selectCompletedOrderDetail2.jsp?orderId="+orderId+"&companyName="+companyName+"&connector="+connector+"&connectorTel="+connectorTel+"&deptUser="+deptUser+"&bookStatus="+bookStatus+"&createTime="+createTime;
		}
