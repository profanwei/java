...
<script src="<%=path %>/resource/timePlanJs/utils.js"></script>
 ...
 <!--    <input id="key" class="mini-textbox" emptyText="请输入订单号" style="width:250px;" onenter="onKeyEnter"/>   
    ...
    <a  class="mini-button" iconCls="icon-undo" plain="false"  onclick="back()">返回</a>
    		<span class="separator"></span> 
        <a  class="mini-button" iconCls="icon-remove" plain="false"  onclick="deletebook()">撤销预约</a>
       ...
       <tr height="60px;">
   		...
   		<div class="mini-fit">
			<div id="tabs1" class="mini-tabs" activeIndex="0" style="width:100%;height:100%;" plain="false"
			    buttons="#tabsButtons">
			    <div title="上报信息" iconCls="icon-node" showCloseButton="false" url="ResearchTask/selectCompletedOrderDetailBehind1.jsp?orderId=<%=request.getParameter("orderId")%>">
			    </div>
      ...
      <script type="text/javascript">
        ...
        function onKeyEnter(e) {
   		...
   		function back(){
   			...
   			var companyNo="${param.companyNo}";
   			
			window.location.href="ResearchTask/sumOrdersSelect.jsp?pageSize="+pageSize+"&pageIndex="+pageIndex+"&status="+status+"&company="+company+"&companyNo="+companyNo;
		}
        function search() {
...
var bookStatus1;
        function deletebook() {
            var orderId="<%=request.getParameter("orderId")%>" ;
        	console.log(orderId);
            mini.confirm("确定删除订单信息？", "确定？",
                function (action) {
                    if (action == "ok") {
       
                    	$.ajax({
                            type:"POST",
                            url:"AuditingBookOrderDelete1.action",               
                            data: {orderId:orderId,bookStatus:bookStatus1},
                            dataType: "json",
