            ...
            <table style="width:100%;">
                <tr>
                    <td style="width:45%;">
                        ...
                        <a class="mini-button" iconCls="icon-remove" onclick="removeRow()" plain="true">未使用上报</a>
                        <span class="separator"></span>
                        <a class="mini-button" iconCls="icon-save" onclick="saveData()" plain="true">完工上报</a>            
                    </td>
        ...
        <div property="columns">
           ...
           <!--  <div type="checkcolumn"></div> -->
            ...
            <div type="comboboxcolumn" autoShowPopup="true" name="machineID" field="machineID" width="100"  alowSrot="false" hideable="true" align="center" headerAlign="center">预约设备
                <input property="editor" class="mini-combobox" style="width:100%;" url="/SSM/MachineNameServlet" />                
            </div>            
        
                              
            <div name="timeYmd" field="timeYmd" width="70"  dateFormat="yyyy-MM-dd" align="center" headerAlign="center" >日期
...
<!--             <div name="orderId" field="orderId" width="100"  >订单号
  ...
  <!--         <div name="completedRemarks"  field="completedRemarks" headerAlign="center"   width="150" >备注
          	...
          	<div type="comboboxcolumn" autoShowPopup="true" name="startTimeInfo" field="startTimeInfo" width="50"  alowSrot="false" hideable="true" align="center" headerAlign="center">开始时间
                <input property="editor" class="mini-combobox" style="width:100%;" data="datelines" />                
            </div> 
          	<div type="comboboxcolumn" autoShowPopup="true" name="endTimeInfo" field="endTimeInfo" width="50"  alowSrot="false" hideable="true" align="center" headerAlign="center">结束时间
                <input property="editor" class="mini-combobox" style="width:100%;" data="datelines" />                
            </div> 
             <div name="passState"  field="passState"   headerAlign="center"  allowSort="true" width="50" align="center" headerAlign="center" renderer="onPassRenderer">上报状态

            </div>
<div type="comboboxcolumn" name="machineStatus"  field="machineStatus"   headerAlign="center"  allowSort="false" width="50" align="center" headerAlign="center" renderer="onmachineRenderer">设备状态
                <input property="editor"  class="mini-combobox" style="width:100%;" minWidth="200" data="machineStatus" />
 
            </div>
                 <div name="comments"  field="comments" autoShowPopup="true"   headerAlign="center"  allowSort="false" width="70" align="center"  renderer="oncommentsRenderer">故障描述
                                 <input property="editor"  class="mini-textarea" style="width:100%;" minWidth="200" value="" />
      ...
      <script type="text/javascript">
        ...
        grid.on("select", function (e) {
            var row = e.record;
            grid.addRowCls(row,"bgcolor");

        }) 
        grid.on("cellbeginedit",function(e){
        	console.log(e.record.name)
   
             if(e.record.name!=="New Row"&&e.field!="machineStatus"){ 
                   e.cancel=true;
        ...
        function onmachineRenderer(e) {
               if (e.value==""||e.value==null) {
            	return e.row.machineStatus="完好";
        }
               else{
            	   return  e.row.machineStatus=e.value;
               }
        }
        function oncommentsRenderer(e) {
            if (e.value==""||e.value==null) {
         	return e.row.comments="";
     }
            else{
         	   return  e.row.machineStatus=e.value;
            }
        ...
        var Genders=[{id: "0", text: "已删除"},{id: "11", text: "待预约"},{id: "12", text: "预约待审核"},{id: "12.5", text: "预约审核中"},{id: "13", text: "待上报"},{id: "14", text: "预约不通过"},{id: "15", text: "上报待审核"},{id: "16", text: "订单完结"}]
        var machineGenders=[{id: "2001001761", text: "数控电火花成形机床"},{id: "2005011533", text: "数控低速走丝电火花线切割"},{id: "2013000822", text: "数控高速成型磨床"},
        	{id: "2016005139", text: "超精密成形平面磨床"},{id: "SK50P", text: "数控车床"},{id: "2008001393", text: "CNC雕刻机"},{id: "2018002921", text: "车铣复合加工中心"},
        	{id: "2007004971", text: "超高速磨床"},{id: "2012000271", text: "超声辅助高速加工中心"},{id: "2001001763", text: "高速五坐标加工中心"},{id: "VL2", text: "加工中心"}]
        ...
        var machineStatus=[{id:'完好',text:'完好'},{id:'故障',text:'故障'}]
       ...
       /*  
        ...
        function addRow() {          
            ...
            grid.beginEditCell(newRow, "machineID");
        }
        ...
        function removeRow() {
            ...
            var data01=mini.encode(rows);
            console.log("sssss"+rows);
            if (rows.length > 0) {
            	 $.ajax({
                     type:"POST",
                     url:"saveCompletedBookingInfo1.action",
        ...
        function onPassRenderer(e) {
            for (var i = 0, l = passGenders.length; i < l; i++) {
                var g = passGenders[i];
   /*          if(parseInt(e.value)==4){
                    e.rowStyle = 'background-color:#93FF93';
                }
                if(parseInt(e.value)==2){
                    e.rowStyle = 'background-color:#FFB5B5';
                }
                if(parseInt(e.value)==3){
                    e.rowStyle = 'background-color:#CA8EFF';
                }
                if(parseInt(e.value)==5){
                    e.rowStyle = 'background-color:#6C6C6C';
                }
                if(parseInt(e.value)==1){
                    e.rowStyle = 'background-color:#FF6347';
                }   */
	    ...
	    function saveData() {
				...
				var data01=mini.encode(data);/*将arrayList序列化为一个字符串 */
				var dataTest=mini.encode({list:data});/* 自己可以在封装其他的参数，类似封装totalCount的时候 */ 
				console.log("tiantian"+data01);
	    ...
	    grid.on("cellendedit",function(e){
	        ...
	        if(e.record.machineID&&e.record.machineID!=""&&e.record.timeYmd&&e.record.timeYmd!=""&&e.record.startTimeInfo&&e.record.startTimeInfo!=""&&e.record.endTimeInfo&&e.record.endTimeInfo!=""){
	        	console.log(e.record.comments);
	        	var timeYmd=e.record.timeYmd;
	        	console.log(timeYmd)
	        	timeYmd=mini.formatDate(mini.parseDate(timeYmd),"yyyy-MM-dd"); 
	        	console.log(timeYmd);
	        	var startTimeInfo=e.record.startTimeInfo.replace(":","-");    
	        	var endTimeInfo=e.record.endTimeInfo.replace(":","-");    
	        	var changedUnid=e.record.machineID+"-"+timeYmd+"-"+startTimeInfo+"-"+endTimeInfo;
	        	console.log(changedUnid);
	        

	        	grid.updateRow(record,{unid:changedUnid,timeYmd:timeYmd});
