...
/* 	<!--
  ...
  <form id="form0">
	    <table >
	   		<tr><td>设备名称：</td>
	          <td><input id="machineName" name="machineName" class="mini-combobox" width="200" textName="" textField="text" valueField="id"  url="/SSM/MachineNameServlet"
  				  allowInput="false" showNullItem="true" nullItemText="全部"  onvaluechanged="loadgrid"/>
	          </td>
	           <td><span></span></td>
	   		  <td><span>&nbsp;</span></td>
	          <td></td>
	          <td><input id="start" name="start"class="mini-datepicker" width="100"  allowinput="false"  format="yyyy-MM-dd" required="true"/>
	          </td>
	          <td><span>至</span></td>
	          <td><input id="end" name="end" class="mini-datepicker"  width="100" allowinput="false"  format="yyyy-MM-dd" required="true"/>
	          </td>
	          <td><span>&nbsp;</span></td>
	          <td><input value="查找" type="button" onclick="loadgrid()" style="width:50px;"/></td>
	           <td><input value="清空" type="button" onclick="remove()" style="width:50px;"/></td>
...
<!--             <div name="action" width="50" headerAlign="center" align="center" renderer="onOperatePower"cellStyle="padding:0;">操作</div> -->
    ...
    <script type="text/javascript">
	    ...
	    $(function(){
	    	...
	    	if("${param.pageIndex}"!=""&"${param.pageSize}"!=""&"${param.machineId}"!=""){
  ...
  function loadgrid(){
	    	...
	    	console.log(data1)
        	if (data1.start>data1.end&&data1.end!==""&&data1.end!==null&&data1.start!==""&&data1.start!==null){
  ...
  function remove(){
	  mini.get("machineName").setValue("");
	  mini.get("start").setValue('');
