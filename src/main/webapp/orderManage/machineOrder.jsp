...
<link rel="stylesheet" type="text/css"
	href="<%=basePath%>css/pagecard.css">
<style type="text/css">
<!--
table {
	table-layout: fixed;
...
<script type='text/javascript'
	src="<%=basePath%>resources/js/tabcard.js"></script>
<!--这个我自己写好的关于订单的状态的js  -->
<%-- 	<script type='text/javascript' src="<%=basePath%>resources/myJs/bookStatusJs.js"></script> --%>
<script type="text/javascript"
	src="<%=basePath%>resources/jquery/jquery.min.js"></script>
<!-- ……………………………………………………………………………………………………………………………………………………………………………………………………………………………… -->
...
<body>
	...
	<div id="grid1" class="mini-datagrid" style="width: 100%; height: 95%;"
		...
		<div property="columns">
			...
			<div name="action" width="50" headerAlign="center" align="center"
				renderer="onOperatePower" cellStyle="padding:0;">操作</div>
			<div field="orderId" width="50" headerAlign="center" align="center">订单编号
			</div>
			<div field="bookStatus" width="60" headerAlign="center"
				align="center" renderer="onGenderRenderer">预约状态</div>
			<div field="companyName" width="50" headerAlign="center"
			...
			<div field="connectorTel" width="60" headerAlign="center"
				align="center">联系人电话</div>
			<div field="productNum" width="50" headerAlign="center"
				align="center">数量</div>
			<div field="material" width="50" headerAlign="center" align="center">材料
			</div>
			<div field="createTime" width="80" headerAlign="center"
				align="center" dateFormat="yyyy-MM-dd HH:mm:ss">订单日期</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function onOperatePower(e) {
			...
			str += "<span>";
			str += "<a style='margin-right:2px;' title='预约设备' href=javascript:ondBook(\'"
					+ e.row.orderId
					+ "\',\'"
					+ e.row.connector
					+ "\',\'"
					+ e.row.bookStatus
					+ "\') ><span class='icon-add' style='width:30px;height:20px;display:inline-block'></span></a>";
			str += "</span>";
			str += "<span>";
		...
		function ondBook(orderId, connector, bookStatus) {
			...
			$.ajax({
				...
				success : function(data) {
					if (data.result == "true") {
						window.location = "OrderSpecServlet?orderId=" + orderId
								+ "&connector=" + connector + "&isModify="
								+ "3";
					} else {
		...
		function ondEdit(orderId, connector) {
			...
			window.location = "OrderSpecServlet?orderId=" + orderId
					+ "&connector=" + connector + "&isModify=" + "3";
		}
		...
		function onQuotation(orderId, connector) {
			window.location = "orderManage/QuotationMainIndex.jsp?orderId="
					+ orderId + "&connector=" + connector;
		}
		/* 
