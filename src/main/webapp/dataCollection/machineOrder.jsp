...
<script src="<%=path %>/resource/timePlanJs/utils.js"></script>
    ...
    <!--style end-->
     <style type="text/css">
	    ...
	    .box-list div{
	    width: 126px;height: 40px;line-height: 40px;text-align: center;
	    float: left;
	...
	margin-left: 17px;
		...
		body{
		background-image: url("<%=path %>/staticResources/image/bgimage.jpg"); 
		}
		article{
		margin-bottom:0px;
...
<script type="text/javascript">
    ...
    <article style="margin-bottom:0px;width:1920px">
        <div class="yuyueBox" style="height:1080px"><!-- 预约内容块开始 -->
            <div class="venue_first">
                <div class="apply" style="width:1900px">
                    ...
                    <div class="img_r" style="float:right"><img src="<%=path %>/resource/images/right.gif"/></div>
                </div>
            </div>
                                                ...
                                                <div class="panel-body">
                                                    <div class="p-tb-xs overflow" id="juzhong">
                                                        <div class="col-xs-5 text-danger text-font12 p-n">
                                                                                              <form id="form0">
	    <table style="margin-left:15px;font-size:8px">
	   		<tr>
	          ...
	          <td><input value="查找" type="button" onclick="loadgrid()" style="width:50px;"/></td>
                                                        ...
                                                        <div class="col-xs-7 p-n">
                                                            <ul class="unstyled overflow text-right m-n p-n pull-right">
                                                                ...
                                                                <li class="pull-left text-right">
                                                                    <img class="m-r-xs"
                                                                         src="<%=path %>/resource/images/no.png">异常
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="article_2"><!-- 方框 -->
                                                        <div class="scrollBox" style="width:1920px"><!-- 方框开始 -->
                                                            <div class="box-scroll" style="width:1920px" id="session_container">
...
<font face="arial" color="black">{{session.bookingUserName}}</font>
																						  	 <br />
...
/*       	var myDate = new Date();
    	var now = myDate.toLocaleDateString().split('/').join('-'); */
    	var time=getNewData(now,17);
    	...
    	$.ajax({
     		type:"POST",
     		url:"<%=path %>/selectmaxorder.action",
     		dataType:"json",
     		data:{begin:now,end:time},
     		success:function(result){
     			
     			console.log(result.machineId);
     			console.log(result.deptId);
     			machineNum=result.machineId-1;
     			deptNum=result.deptId-1;
    ...
    function initAllVenues() {
        U.ajax({
            url: "/SSM/ShowDeptId.action",// 请求URL
            data: {},
            loading: true,
            success: function (data, status) {
                if (data.no == 2000) {
                    ...
                    /* if (U.isEmpty(curVenuesId)) { */
                        //默认选中第一个场馆
                        $('.apply_w').find('a').eq(deptNum).trigger('click');
                 /*    } else { 
             ...
             success: function(data){
            	 if (data.no == 2000) {
                    ...
                    /*  if(U.isEmpty(machineId)){ 
                    	 ...
                    	 $('#dataLine_container').find('a').eq(machineNum).trigger('click');
                   /*   }  */
