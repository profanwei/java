...
<html>
  ...
  -->
  <div>
  	 <input id="username" name="username" class="mini-textbox" onenter="Search()" emptyText="请输入工号"/><%-- <span><%=message%></span> --%>
    ...
    <script type="text/javascript">
	   ...
	   /*  function search() {
	    ...
	    function onOperatePower(e) {
	        ...
	        str += "<a style='margin-right:2px;' title='删除' href=javascript:ondelete(\'" + e.row.userId+"\',\'"+e.row.staffCode+"\') ><span class='icon-remove' style='width:30px;height:20px;display:inline-block'></span></a>";
	        str += "</span>";
	    ...
	    function ondEdit(userId){
	        window.location.href = "EditSysUsersServlet?para=0&userId="+userId;
		}
		
		function ondelete(userId,staffCode){	//参数1 表示删除用户
			mini.confirm("删除当前用户", "提示",
                function (action){            
                  if (action == "ok") {
